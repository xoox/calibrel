...
public:
    ...
    void validate()
    {
        ...
        if (calibrationPattern == NOT_EXISTING) {
            cerr << " Camera calibration mode does not exist: "
                 << patternToUse << endl;
            goodInput = false;
        }
        atImageList = 0;
...
enum CALIB_MODE {
    DLR11, //!< The method implemented in this project
    STD, //!< The standard method of OpenCV
    HYBRID //!< STD followed by DLR11 method
};
...
int main(int argc, char* argv[])
{
    ...
    // FileStorage fout("settings.yml", FileStorage::WRITE); // write config
    // as YAML
    // fout << "Settings" << s;
    ...
    for (;;) {
        ...
        case Settings::CHESSBOARD:
            found = findChessboardCorners(
                view, s.boardSize, pointBuf, chessBoardFlags);
            break;
        ...
        case Settings::ASYMMETRIC_CIRCLES_GRID:
            found = findCirclesGrid(
                view, s.boardSize, pointBuf, CALIB_CB_ASYMMETRIC_GRID);
            break;
        default:
            found = false;
            break;
        ...
        if (mode == CAPTURING) {
            if (s.showUndistorsed)
                msg = format(
                    "%d/%d Undist", (int)imagePoints.size(), s.nrFrames);
            else
                msg = format("%d/%d", (int)imagePoints.size(), s.nrFrames);
        }

        putText(
            view, msg, textOrigin, 1, 1, mode == CALIBRATED ? GREEN : RED);
        ...
        if (mode == CALIBRATED && s.showUndistorsed) {
            ...
            if (s.useFisheye)
                cv::fisheye::undistortImage(
                    temp, view, cameraMatrix, distCoeffs);
            else
                undistort(temp, view, cameraMatrix, distCoeffs);
        }
        //! [output_undistorted]
        //------------------------------ Show image and check for input
        // commands -------------------
        //! [await_input]
    ...
    if (s.inputType == Settings::IMAGE_LIST && s.showUndistorsed) {
        ...
        if (s.useFisheye) {
            ...
            fisheye::initUndistortRectifyMap(cameraMatrix, distCoeffs,
                Matx33d::eye(), newCamMat, imageSize, CV_16SC2, map1, map2);
        } else {
    ...
    CALIB_MODE calib_mode = DLR11)
{
    ...
    if (s.useFisheye) {
        ...
        distCoeffs = Mat::zeros(12, 1, CV_64F);
    }
    ...
    if (s.useFisheye) {
        ...
        cout << "Calibration phase execution time is "
             << (t1 - t0) / (double)getTickFrequency() << " seconds" << endl;
        cout << "New board corners: " << endl;
        cout << newObjPoints[0].x << " " << newObjPoints[0].y << " "
             << newObjPoints[0].z << endl;
        cout << newObjPoints[s.boardSize.width - 1].x << " "
             << newObjPoints[s.boardSize.width - 1].y << " "
             << newObjPoints[s.boardSize.width - 1].z << endl;
        cout << newObjPoints[s.boardSize.width * (s.boardSize.height - 1)].x
             << " "
             << newObjPoints[s.boardSize.width * (s.boardSize.height - 1)].y
             << " "
             << newObjPoints[s.boardSize.width * (s.boardSize.height - 1)].z
             << endl;
        cout << newObjPoints.back().x << " " << newObjPoints.back().y << " "
             << newObjPoints.back().z << endl;
    }
    ...
    totalAvgErr = computeReprojectionErrors(objectPointsArray, imagePoints,
    ...
    const vector<vector<Point2f> >& imagePoints, double totalAvgErr)
{
    ...
    if (s.flag) {
        ...
        if (s.useFisheye) {
            ...
            flagsStringStream
                << "flags:" << (s.flag & CALIB_USE_INTRINSIC_GUESS
                                       ? " +use_intrinsic_guess"
                                       : "")
