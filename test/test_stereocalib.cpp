...
/* *************** License:**************************
   Oct. 3, 2008
   Right to use this code in any way you want without warranty, support
   or any guarantee of it working.
   ************************************************** */
...
static int print_help()
{
    cout << " Given a list of chessboard images, the number of corners (nx, "
            "ny) on the chessboards, and a flag: useCalibrated for "
            "calibrated (0) or uncalibrated (1: use cvStereoCalibrate(), 2: "
            "compute fundamental matrix separately) stereo.  Calibrate the "
            "cameras and display the rectified results along with the "
            "computed disparity images.\n"
         << endl;
    cout << "Usage:\n test_stereocalib -w=<board_width default=9> "
            "-h=<board_height default=6> -s=<square_size default=1.0> <image "
            "list XML/YML file default=stereo_calib.xml>\n"
         << endl;
    return 0;
}

static void StereoCalib(const vector<string>& imagelist, Size boardSize,
    float squareSize, bool displayCorners = false, bool useCalibrated = true,
    bool showRectified = true)
{
    ...
    double rms = calrel::stereoCalibrate(objectPoints, imagePoints[0],
        imagePoints[1], cameraMatrix[0], distCoeffs[0], cameraMatrix[1],
        distCoeffs[1], imageSize, R, T, E, F, CALIB_FIX_ASPECT_RATIO
    ...
    // from the fundamental matrix
    {
        ...
        F = findFundamentalMat(
            Mat(allimgpt[0]), Mat(allimgpt[1]), FM_8POINT, 0, 0);
        Mat H1, H2;
        stereoRectifyUncalibrated(
            Mat(allimgpt[0]), Mat(allimgpt[1]), F, imageSize, H1, H2, 3);
